...
describe("users", () => {
  ...
  test("correct count of ready-rest users & ready-icon of rest users shown", () => {
    // 6 online users -> display 4 & rest user count of 2
    const newState3 = {
      users: {
        all: [
          ...offlineUsers,
          ...adminWithOtherOnlineUsers.slice(0, 2), // +1 additional online user
        ],
      },
    };
    const newBoardUsers3 = render(createBoardUsers(newState3));
    ...
    // check icon displaying rest-users
    expect(newBoardUsers3.container.querySelectorAll(".rest-users__ready-icon")).toHaveLength(1);
  });
});
